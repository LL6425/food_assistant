...
class StructOutput:
    ...
    def __init__(self):
        assert (
            isinstance(self.indent, int) and self.indent >= 0
        ), "Indent must be a non-negative integer."
        assert issubclass(
            self.output_structure, BaseModel
        ), "Output structure must be a subclass of pydantic BaseModel."
    ...
    def require_struct_output(
        ...
        return history + [
            {"role": "system", "content": self._get_system_add()},
...
class Guardian(StructOutput):
    ...
    def complete_history(
        ...
        return history + [
            {
                "role": "system",
                "content": self.guardian_prompt.system_description
                + " "
                + self._get_system_add(),
            },
            {
                "role": "user",
